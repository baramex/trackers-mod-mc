...
import ch.baramex.trakersmod.tools.ServerM;
...
public class TileEntityTrackerAdv extends TileEntity implements ITickable {
	
	public static final int ANTENNE_MIN = 5;
	public static final int ANTENNE_MAX = 10;
	public static final int ENERGY_CONSUME = 500000;
	public static final int MIN_BLOCK = 3500;
	public static final int MAX_BLOCK = 5000;
	public static final int MIN_PRECISION_BLOCK = 5;
	public static final int MAX_PRECISION_BLOCK = 50;
	...
	public ItemStack playerItemStack = ItemStack.EMPTY;
	public int work = 0;
	public int work1 = 0;
	...
	public Map<String, ItemStack> clientItemStack = new HashMap<String, ItemStack>();
	
	public ItemStack getClientItemStack(String name) {
		if(this.clientItemStack.get(name) == null) {
			return new ItemStack(Blocks.BARRIER, 1);
		}
		else {
			return this.clientItemStack.get(name);
		}
	}
	
	public void setClientItemStack(ItemStack pl, String name) {
		if(this.clientItemStack.containsKey(name)) {
			this.clientItemStack.replace(name, pl);
		}
		else {
			this.clientItemStack.put(name, pl);
		}
	}
	...
	public void update() {
		if(!world.isRemote) {
			if(work1 == 0) {
				...
				if(playerItemStack != ItemStack.EMPTY) {
					playerItemStack = ItemStack.EMPTY;
				}
			}
			else {
				work1--;
			...
			if(requestTrack && isGood == "Ready for tracking") {
				...
				if(player == null && playerName != null) {
					...
					playerName = null;
					playerItemStack = player.inventory.getCurrentItem();
					if(playerItemStack == null || playerItemStack == ItemStack.EMPTY) {
						playerItemStack = new ItemStack(Blocks.BARRIER, 1);
					}
					work = 20;
					requestTrack = false;
					// 1 / 4
