...
public class ServerB extends Block implements ITileEntityProvider {
	
	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	
	List<String> tooltip = Arrays.asList(new String[]{TextFormatting.DARK_GREEN + new TextComponentTranslation("tooltip." + Reference.MODID + ".server.1").getFormattedText(), TextFormatting.YELLOW + new TextComponentTranslation("tooltip." + Reference.MODID + ".server.2").getFormattedText()});

	public ServerB(Material materialIn, String name) {
		super(materialIn);
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		TileEntityServer tileentity = (TileEntityServer) worldIn.getTileEntity(pos);
		
		if(active) worldIn.setBlockState(pos, ModBlocks.server.getDefaultState().withProperty(FACING, state.getValue(FACING)), 3);
		else worldIn.setBlockState(pos, ModBlocks.server.getDefaultState().withProperty(FACING, state.getValue(FACING)), 3);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!worldIn.isRemote && te instanceof TileEntityServer) {
