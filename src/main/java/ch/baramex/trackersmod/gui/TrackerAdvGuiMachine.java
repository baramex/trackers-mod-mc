...
public class TrackerAdvGuiMachine extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        fontRenderer.drawString(TextComponents.getTileName("trackerAdv").getFormattedText(), 5 + guiLeft, 5 + guiTop, Color.BLACK.getRGB());
    	...
    	if(result != "" && tile.getClientItemStack() != null) {
    		drawCenteredString(fontRenderer, TextComponents.coordinate.getFormattedText(), guiLeft+xSize/2-40, guiTop+90, Color.GREEN.getRGB());
    		drawCenteredString(fontRenderer, result, guiLeft+xSize/2-40, guiTop+100, Color.GREEN.getRGB());
    		
    		GlStateManager.enableLight(0);
    		RenderItem renderItem = mc.getRenderItem();
			renderItem.renderItemIntoGUI(tile.getClientItemStack(), guiLeft + xSize/2+40, guiTop + 90);
			drawCenteredString(fontRenderer, "x" + tile.getClientItemStack().getCount(), guiLeft+xSize/2+67, guiTop+94, Color.GREEN.getRGB());
