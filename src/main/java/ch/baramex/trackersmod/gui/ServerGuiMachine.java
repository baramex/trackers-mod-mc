...
public class ServerGuiMachine extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        drawEnergyBar(tile.getClientInt("energy"));
        
        fontRenderer.drawString(TextComponents.getTileName("server").getFormattedText(), 5 + guiLeft, 5 + guiTop, Color.BLACK.getRGB());
        
        fontRenderer.drawString(tile.getClientInt("antenne") + " " + TextComponents.antenna.getFormattedText() + "(s)" , 90 + guiLeft, 40 + guiTop, tile.getClientInt("antenne") <= 0 ? Color.RED.getRGB() : tile.getClientInt("antenne") <= 2 ? Color.ORANGE.getRGB() : Color.BLACK.getRGB());
        fontRenderer.drawString(tile.getClientInt("nbMachine") + " machine(s)" , 90 + guiLeft, 70 + guiTop, tile.getClientInt("nbMachine") <= 0 ? Color.RED.getRGB() : Color.BLACK.getRGB());
